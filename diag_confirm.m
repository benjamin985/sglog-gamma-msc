...
cfg = dataset_config(root_dir, 'uci'); % placeholder for default
ds_list = {'uci','orl','bbc','yale','scene'};
l1s = [1e-4, 5e-4];
...
for di = 1:numel(ds_list)
    ...
    fprintf('== confirm %s N=%d V=%d K=%d (elementwise, full-kNN k=16) ==\n', ds, N, numel(X), K);
    for a = 1:numel(l1s)
    for b = 1:numel(l2s)
    for c = 1:numel(gs)
        ...
        variant.error_penalty = 'elementwise_log';
        [model, history] = solve_sglog(X, Q, cfg.parameters, variant);
        W = model.affinity;
        nmi = spectral_nmi(W, gt, K);
        Z1 = abs(model.Z{1}); off = Z1; off(1:N+1:end) = 0;
        fprintf('  l1=%.0e l2=%.0e g=%d | NMI=%.3f | meanDiag=%.3g meanOff=%.3g maxOff=%.3g | %s it=%d\n', ...
            l1s(a), l2s(b), gs(c), nmi, mean(diag(Z1)), mean(off(:)), max(off(:)), ...
